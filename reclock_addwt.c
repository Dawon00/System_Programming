	...
	while (1) {
		...
		switch (operation) {
			...
			case 'w' : // withdraw
				...
				printf("enter amount\n");
				scanf("%d", &amount);
				if(current.balance < amount){ //can't withdraw
				...
				break;
			case 't' : // transfer
				reclock(fd, record_no, sizeof(struct record), F_WRLCK);//lock
				pos = record_no * sizeof(struct record);
				lseek(fd, pos, SEEK_SET);
				...
				current.balance -= amount; //withdraw
				
				
				lseek(fd, pos, SEEK_SET);
				write(fd, &current, sizeof(struct record));
				
				printf("enter account number to send (0-99): "); //transfer to
				scanf("%d", &record_no);
				fgets(buffer, 100, stdin);
				if (record_no < 0 && record_no >= NUM_RECORDS)
					break;
